...
def loss_mse_hetero(y_true, y_pred):
    ...
    """
    return K.exp(
        K.mean(K.log(K.mean(K.square(y_pred - y_true), axis=0)), axis=-1))


def build_masked_loss(loss_function=loss_mse_hetero, mask_value=MISSING_LABEL_FLAG):
...
def build_model(loss=build_masked_loss(), embed_size=EMBEDDING_SIZE, num_classes=6):
    ...
    model.compile(loss=loss, optimizer='adam', metrics=['accuracy', masked_accuracy])
    return model
